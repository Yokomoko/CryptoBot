...
namespace CryptoBot.Code.Connection
{
    public class Connection
    {
        ...
        public static List<MarketSummary> MarketSummaries { get; set; } = new List<MarketSummary>();
        ...
        public static async Task Populate()
        {
            ...
            NotableCurrenciesList.Clear();
            MarketSummaries.Clear();

            var k = StringCipher.Decrypt(secureKey);
            var s = StringCipher.Decrypt(secureSecret);
            ...
            if (summary.Success)
            {
                ...
                MarketSummaries = summaries.OrderBy(d => d.MarketName == "USDT-BTC" ? 0 : 1).ToList();
            }
            else
            {
        ...
        {
            NotableCurrenciesList.Add("USDT-BTC");
            //Notable Currencies
            foreach (var balance in AccountBalances)
            {
                if (!NotableCurrenciesList.Contains(balance.Currency))
                {
                    NotableCurrenciesList.Add("BTC-" + balance.Currency);
                }
            }
            foreach (var order in OpenOrders)
            {
                ...
                {
                    if (!NotableCurrenciesList.Contains(c))
                    {
                        NotableCurrenciesList.Add("BTC-" + c);
                    }
                }
            }
        }
